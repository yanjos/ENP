...
$(".menu-page-b").children().wrapAll('<div class="custom-wrapped-menu"/>');

/*Custom Wrap for page-b areas so modals can be configured to go into it, as all *-page-b vary in names we need a consistent name, enp-body which we will use as class in backed config as modal container*/
if(!$('.enp-body-modals').length){
...
$(document).on("dialogopen", ".ui-res-modal", function(event, ui) {
    ...
    //Wrapped in a setTimeout as the POST request needs a but of time to be retrieved and have the correct count of ui-dialog - so we can apply out currentModal/currentAdditionalModal classes correctly and show/hide what we need
...
$(document).on( "dialogclose", function( event, ui ) {
    // Show custom wrapped menu (menu page) incase it was hidden on enp-modal-body-close
    $('.custom-wrapped-menu').show();
...
$(window).load(function() {
    ...
    if(!$('.ui-res-menu-accordion-container:visible').length){
        $('.selected-menu-category-title:first').prepend('<a href="/myres/menu/menu.jsp?RESET_MENU=Y" class="ui-btn go-back-chevron"><i class="fa fa-caret-left"></i></a>');
    }else{
        $('.selected-category-description').hide();
        $('.selected-menu-category-title:first').prepend('<a href="/myres/locations/searchLocationsResults.jsp" class="ui-btn go-back-chevron"><i class="fa fa-caret-left"></i></a>');
    }
